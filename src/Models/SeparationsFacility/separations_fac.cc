...
void SeparationsFac::phase(SeparationsFac::Phase p) {
  LOG(cyclus::LEV_DEBUG2, "FCOFF") << "SeparationsFac " << name()
                                    << " is changing phases -";
...
void SeparationsFac::BeginProcessing_() {
  LOG(cyclus::LEV_DEBUG2, "FCOFF") << "SeparationsFac " << name() << " added"
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
cyclus::Composition::Ptr SeparationsFac::GoalComp_(){
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
cyclus::Material::Ptr SeparationsFac::CollapseBuff(cyclus::ResourceBuff to_collapse){
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void SeparationsFac::MoveToStocks_(cyclus::ResourceBuff fabbed_fuel_buff, int n_poss){
  using cyclus::Manifest;
  using cyclus::Material;
  using cyclus::ResCast;
...
void SeparationsFac::FabFuel_(){
  ...
  if( n > 0 ){ 
    ...
    LOG(cyclus::LEV_DEBUG2, "FCOFF") << "SeparationsFac " << name() << " is fabricating fuel.";
  }
}
...
SeparationsFac::GetOrder_(double size) {
  ...
  for (it = commods.begin(); it != commods.end(); ++it) {
    ...
    LOG(cyclus::LEV_DEBUG3, "FCOFF") << "SeparationsFac " << name()
                                      << " is making an order:";
